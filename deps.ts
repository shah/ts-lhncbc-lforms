export * as colors from "https://deno.land/std@0.79.0/fmt/colors.ts";
export * as fs from "https://deno.land/std@0.79.0/fs/mod.ts";
export * as path from "https://deno.land/std@0.79.0/path/mod.ts";
export * as govnDataCLI from "https://denopkg.com/gov-suite/governed-structured-data@v0.7.7/cli.ts";
export * as govnData from "https://denopkg.com/gov-suite/governed-structured-data@v0.7.7/mod.ts";
export * as inflect from "https://denopkg.com/shah/text-inflect@v1.0.5/mod.ts";
export * as inspect from "https://denopkg.com/shah/ts-inspect@v0.8.8/mod.ts";
export * as jsonMutator from "https://denopkg.com/shah/ts-json-mutator@v0.1.1/mod.ts";
export * as safety from "https://denopkg.com/shah/ts-safety@v0.3.0/mod.ts";
export * as jsonPatch from "https://denopkg.com/Starcounter-Jack/JSON-Patch@3.0.0-1/index.mjs";
